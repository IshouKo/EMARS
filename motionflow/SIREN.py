...
class SineLayer(nn.Module):
    ...
    # 従来のニューラルネットワークがReLUなどの非線形活性化関数を使用するのに対し、Sirenはすべての隠れ層で正弦関数を使用
    # この正弦関数は無限に微分可能であるため、ネットワークは非常に滑らかで詳細なデータの表現を学習することができる
    # さらに、Sirenは高周波数の詳細を効果的にキャプチャするため、画像や音声などの複雑な信号のモデリングに特に適している
